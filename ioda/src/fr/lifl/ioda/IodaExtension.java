...
public class IodaExtension extends org.nlogo.api.DefaultClassManager {
	...
	Iterator<Object> it = Matrix.activeBreedsWithTargets().iterator() ;
	while(it.hasNext()) {
	    AgentSet s = (AgentSet) it.next() ;
	...
	for (Assignation a: assign) {
	    ...
	    l.addAll(i.getTrigger()) ;
	    l.addAll(i.getCondition()) ;
	    for (Object reps : l) {
		LogoList lreps = (LogoList) reps ;
		for (Object rep: lreps) {
    ...
    //----------------------------------------------------------------
    public static class Version  extends DefaultReporter {
    ...
    //----------------------------------------------------------------
    public static class GetBreedNamed  extends DefaultReporter {
	public Syntax getSyntax() {
	    return Syntax.reporterSyntax(new int[] {Syntax.StringType()}, Syntax.AgentsetType()|Syntax.NobodyType()) ;
	}
	public String getAgentClassString() { return "OTPL" ; }
	public Object report(Argument args[], Context context) 
	    throws ExtensionException, LogoException {
    ...
    //----------------------------------------------------------------
    public static class CheckConsistency  extends DefaultReporter {
	public Syntax getSyntax() {
	...
	public Object report(Argument args[], Context context) 
	    throws ExtensionException, LogoException {
	    return checkConsistency(((ExtensionContext)context).workspace().getProcedures().keySet()) ;
	}
    }

    //----------------------------------------------------------------
    public static class PrimitivesToWrite  extends DefaultReporter {
	public Syntax getSyntax() {
	    return Syntax.reporterSyntax(new int[] {}, Syntax.StringType()) ;
	}
	public String getAgentClassString() { return "OTPL" ; }
	public Object report(Argument args[], Context context) 
	    throws ExtensionException, LogoException {
	    return printPrimitives(((ExtensionContext)context).workspace().getProcedures().keySet()) ;
	}
    }
