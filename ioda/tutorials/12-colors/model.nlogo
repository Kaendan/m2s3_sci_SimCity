...
to create-new-assignation
  let source one-of ["azure" "gules" "silver" "gold"]
  let target one-of ["nobody" "azure" "gules" "silver" "gold" "azure" "gules" "silver" "gold"]
  let prio random 100
...
to-report azure::empty-neighbor?
...
to-report silver::empty-neighbor?
  report empty-neighbor?
end

to-report gold::empty-neighbor?
  report empty-neighbor?
end

to azure::move-to-neighbor
  move-to-neighbor
end
...
to silver::move-to-neighbor
  move-to-neighbor
end

to gold::move-to-neighbor
  move-to-neighbor
end

to azure::filter-neighbors
  filter-neighbors
end
...
to silver::filter-neighbors
  filter-neighbors
end

to gold::filter-neighbors
  filter-neighbors
end
...
to gold::clone-or-let-clone
  clone-or-let-clone
end

to azure::clone-or-let-clone
  clone-or-let-clone
end
...
to silver::clone-or-let-clone
  clone-or-let-clone
end

to azure::die-or-kill
  die-or-kill
end

to silver::die-or-kill
  die-or-kill
end
...
to gold::become-white
  become-white
end

to azure::become-white
  become-white
end
...
to silver::become-yellow
  become-yellow
end

to azure::become-yellow
  become-yellow
end

to gold::become-yellow
end
@#$#@#$#@
GRAPHICS-WINDOW
423
...
253
add one
without-interruption [\n  let p (patches with [not any? turtles-here])\n  if (any? p) \n    [ ask one-of p \n      [ sprout-gold 1 [ gold::init ioda:init-agent ]\n      ]\n]]
NIL
1
T
OBSERVER
...
288
add one
without-interruption [\n  let p (patches with [not any? turtles-here])\n  if (any? p) \n    [ ask one-of p \n      [ sprout-silver 1 [ silver::init ioda:init-agent ]\n      ]\n]]
NIL
1
T
OBSERVER
...
"" ""
PENS
"Azure" 1.0 0 -13345367 true "" "plot count azure"
"Gules" 1.0 0 -2674135 true "" "plot count gules"
"Silver" 1.0 0 -7500403 true "" "plot count silver"
"Gold" 1.0 0 -1184463 true "" "plot count gold"
...
665
matrix-lines
azure  CloneSource  10\nazure  ChangeToGold  20  azure  1 NUMBER:6-\nazure  KillTarget   15  gules  1  ALL\nazure  KillTarget   15  silver  1  ALL\n
1
1
String
...
This model presents the IODA NetLogo adaptation of a demo applet which is available from the IODA Webpage (http://cristal.univ-lille.fr/SMAC/projects/ioda/). It consists in generic entities, identified by their color, which can do exactly the same actions. They are endowed with "generic" interactions, such as `CloneSource`, `Wander`, `KillTarget`, etc.   
