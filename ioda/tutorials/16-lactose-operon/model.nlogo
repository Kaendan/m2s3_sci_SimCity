...
to init-cell
  ...
  set-default-shape lactose "lactose"
  set-default-shape glucose "glucose"
  set-default-shape permeases "permease"
  set-default-shape galactosidases "galactosidase"
  set-default-shape CAP "cap"
  set-default-shape cAMP "camp"
  set-default-shape ATP "atp"
  create-cell
end
...
to create-cell
  ...
  ask membranes [ set color gray set binds-with task [ permeases ]  ]
  build-operon
  create-polymerases 20 [ polymerases::init ]
  create-repressors 10 [ repressors::init ]
  create-CAP 10 [ cap::init ]
  create-cAMP 20 [ camp::init] 
  create-ATP 20 [ atp::init] 
...
to build-operon
  ...
  ask genes-on patch 1 0 [ set breed operators set color orange set binds-with task [ repressors ]]
  ...
  ask genes-on patch -1 0 [ set breed operators set color orange set binds-with task [ CAP with [ complexed? ] ]]
...
to add-lactose [ nb ]
  create-lactose nb [ lactose::init ]
end

to add-glucose [ nb ]
    create-glucose nb [ move-to one-of patches with [not inside? and not any? membranes-here] glucose::init ]
end
...
to-report out-lactose
  report  (count lactose-on patches with [not inside?]) / (count patches with [not inside?])
end

to-report out-glucose
  report  (count glucose-on patches with [not inside?]) / (count patches with [not inside?])
end

to-report in-lactose
  report  (count lactose-on patches with [inside?]) / (count patches with [inside?])
end

to-report in-glucose
  report  (count glucose-on patches with [inside?]) / (count patches with [inside?])
end
...
to-report lactose::can-move?
  report not any? membranes-on patch-ahead speed
end

to-report lactose::time-to-turn?
  report random 100 < 5
end
...
to lactose::random-turn
  wiggle 60
end

to-report lactose::chance-to-cross?
...
to-report operators::occupied?
  ...
  report (any? repressors-here with [self != t]) or (any? CAP-here with [self != t])
end
...
to repressors::break-complex
  hatch-lactose 1 [ move-to one-of neighbors ioda:init-agent]
  set complexed? false
  set shape "repressor"
end

to-report rna::chance-for-translation?
  report random 100 < 1
end

to-report rna::can-move?
  report [inside?] of patch-ahead speed
end

to-report rna::time-to-turn?
  report random 100 < 5
end
...
to genes::build-RNA
  hatch-rna 1 [ set heading random 360 set speed 0.1 fd speed ioda:init-agent ]
end
...
to-report glucose::can-move?
  report not any? membranes-on patch-ahead speed
end

to-report glucose::time-to-turn?
  report random 100 < 5
end
...
to-report atp::can-move?
  report [inside?] of patch-ahead speed
end

to-report atp::time-to-turn?
  report random 100 < 5
end

to-report camp::can-move?
  report [inside?] of patch-ahead speed
end

to-report camp::time-to-turn?
  report random 100 < 5
end

to-report cap::can-move?
  report [near-dna?] of patch-ahead speed
end

to-report cap::time-to-turn?
  report random 100 < 5
end
...
to atp::die
  hatch-camp 1  [ set color blue - 2 ioda:init-agent ]
  ioda:die
end

to-report camp::complexed?
  report false 
end

to glucose::filter-neighbors
  ioda:filter-neighbors-on-patches (patch-set patch-here patch-ahead 1)
end

to camp::make-complex
  hatch-atp 1 [ set lifespan random-poisson 300 set color blue + 2 ioda:init-agent ]
  ioda:die
end
...
to-report glucose::chance-to-cross?
  report (random-float 1000 < natural-permeability) and (not inside?)
end

to glucose::move-to-other-side
 let x [xcor] of ioda:my-target
 let y [ycor] of ioda:my-target
 setxy (xcor + (x - xcor) * 2) (ycor + (y - ycor) * 2)  
end

to-report cap::complexed?
  report complexed?
end

to camp::filter-neighbors
  ioda:filter-neighbors-on-patches (patch-set patch-here patch-ahead 1)
end
...
to-report cap::chance-to-dissociate?
  report random 1000 < 10
end

to cap::break-complex
  hatch-camp 1 [ set color blue - 2 move-to one-of neighbors ioda:init-agent ]
  set complexed? false
  set bound? false
  set shape "cap"
end
...
to-report cap::bad-conformation?
  report false
end

to-report glucose::chance-to-unbind?
  report true
end

to-report cap::promoter-nearby?
...
"" ""
PENS
"out-lactose" 1.0 0 -8431303 true "" "plot (count lactose-on patches with [not inside?]) / (count patches with [not inside?])"
"in-lactose" 1.0 0 -1184463 true "" "plot (count lactose-on patches with [inside?]) / (count patches with [inside?])"
"out-glucose" 1.0 0 -10141563 true "" "plot (count glucose-on patches with [not inside?]) / (count patches with [inside?])"
"in-glucose" 1.0 0 -13345367 true "" "plot (count glucose-on patches with [inside?]) / (count patches with [inside?])"
...
43
clear lactose
ask lactose with [ioda:alive?] [ioda:die]
NIL
1
T
OBSERVER
...
"" ""
PENS
"ATP" 1.0 0 -8020277 true "" "plot count ATP"
"cAMP" 1.0 0 -15390905 true "" "plot count cAMP"
...
82
clear glucose
ask glucose with [ioda:alive?] [ioda:die]
NIL
1
T
OBSERVER
...
Web Site: http://cristal.univ-lille.fr/SMAC/projects/ioda
