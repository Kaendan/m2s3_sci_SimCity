...
to init-world
  ...
  set-current-plot "World summary"
  foreach teams 
    [ create-temporary-plot-pen (word "team " ?) set-plot-pen-color ?]
...
to place-fish
  ask n-of 10 patches with [pcolor = turquoise]
    [ sprout-fish 1 [ init-fish set heading random 360 fd 0.1 + random-float 0.3 ]]
end
...
;to-report fish::not-decomposed?
;  report grown? and quantity > 0
;end

to-report fish::decomposed?
...
;to-report fish::not-dead?
;  report true
;end

to-report fish::depleted?
  report quantity < 500
end

to-report fish::grown?
  report grown?
end
...
to-report fire::windy-weather?
  report true
end

to fire::filter-neighbors
...
to fire::ignite
  ...
  foreach pp 
    [let p [patch-here] of ?
     ask p [ if not any? fire-here [sprout-fire 1 [ init-fire ]]]]
end
